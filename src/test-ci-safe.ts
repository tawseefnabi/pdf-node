...
generatePDF(undefined as any).catch((err: Error) => {
...
generatePDF({} as any).catch((err: Error) => {
	if (err.message && err.message.includes('options are missing')) {
...
const testMissingFields = async () => {
	...
	// Test missing data
	try {
		await generatePDF({html: '<div>test</div>'} as any);
...
// Run the tests
testMissingFields().catch((err: Error) => {
